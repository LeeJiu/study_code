...
void bullet2::fire(float x, float y)
{
	if (_bulletMax < _vBullet.size()) return;
...
HRESULT bullet3::init(const char * imageName, int bulletMax, float range)
{
	_imageName = imageName;
	_bulletMax = bulletMax;
	_range = range;
...
void bullet3::fire(float x, float y)
{
	if (_bulletMax < _vBullet.size()) return;
...
void bullet3::removeBullet(int arrNum)
{
	_vBullet.erase(_vBullet.begin() + arrNum);
}